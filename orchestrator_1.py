...
ШАГ 2: ПОИСК ЗАДАЧ В JIRA (ОБЯЗАТЕЛЬНО СДЕЛАЙ НЕ МЕНЕЕ 2 ЗАПРОСОВ)
Для честного анализа НЕ ОГРАНИЧИВАЙСЯ статусом самих эпиков! Ты ОБЯЗАН анализировать вложенные задачи:

1. Запрос по дочерним задачам целей: Извлеки ключи эпиков/целей из файла (например, OT-291, OT-303) и выполни JQL с оператором parent:
   `parent IN (OT-291, OT-303, OT-313) OR issueKey IN (OT-291, OT-303, OT-313)`
   (Если ключей нет, используй ключевые слова из целей через summary ~ "..." OR text ~ "...").

2. Запрос по внеплановым задачам и багам: Выполни отдельный JQL для поиска незапланированной активности:
   `summary ~ "внеплан*" OR text ~ "внеплан*" OR summary ~ "BUG" OR summary ~ "КРИТИЧНО" OR parent IN (OT-320)`
...
### Внеплановые задачи
- Перечисли все найденные внеплановые задачи, критические баги и инциденты.
- Если таких задач нет — напиши ровно: "Внеплановых задач не обнаружено".

### Список незавершенных задач-кандидатов на перенос
- Выдели незавершенные задачи (со статусами To Do, In Progress, In Review, Testing), у которых есть блокеры, задержки или специфические комментарии (например, отпуск, ожидание ручек от бэкенда, проблемы с тестами).
- Для каждой такой задачи кратко укажи ключ, название и причину задержки/переноса.
